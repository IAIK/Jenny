...
static int domain = -1;
static int file_is_private = 0;
extern void pk_exception_handler();
extern void sf_localstorage_filters_init(int did);
GEN_ALL(ngx_http_auth_basic_handler_private, static ngx_int_t, ngx_http_request_t *r, ngx_http_auth_basic_loc_conf_t  *alcf, ngx_str_t realm, ngx_str_t user_file)
...
__attribute__ ((used))
static ngx_int_t
ngx_http_auth_basic_handler_private(ngx_http_request_t *r, ngx_http_auth_basic_loc_conf_t  *alcf, ngx_str_t realm, ngx_str_t user_file)
{
    off_t                            offset;
    ssize_t                          n;
    ngx_fd_t                         fd;
    ...
    //fprintf(stderr, "ngx_http_auth_basic_handler_private:%d (%p, %p, %p, %p)\n", __LINE__, r, alcf, &realm, &user_file);
    //ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_http_auth_basic_handler_private %d", __LINE__);

    if(domain != -1 && !file_is_private){
        int ret = pk_api_generic_register_private_file(PK_DOMAIN_CURRENT, (char*)user_file.data);
        assert(ret == 0);
        file_is_private = 1;
    }
...
ngx_http_auth_basic_handler(ngx_http_request_t *r)
{
    ...
    if(domain == -1){
        return ngx_http_auth_basic_handler_private(r, alcf, realm, user_file);
    }else{
        return ECALL(ngx_http_auth_basic_handler_private(r, alcf, realm, user_file));
    }
...
ngx_http_auth_basic_init(ngx_conf_t *cf)
{
    ...
    *h = ngx_http_auth_basic_handler;
    ...
    //ngx_http_auth_basic_loc_conf_t  *alcf = ngx_http_conf_get_module_main_conf(cf, ngx_http_auth_basic_module);
    //ngx_http_auth_basic_loc_conf_t *alcf = cf;
    //assert(alcf);
    //fprintf(stderr, "alcf->user_file = %s\n", (char*)alcf->user_file.value.data);

    char* mech = getenv("MECHANISM");
    char* filter = getenv("FILTER");
    if ((mech && strcmp(mech, "none") != 0) || (filter && strcmp(filter, "none") != 0)) {
        //ngx_log_error(NGX_LOG_ERR, TODO, 0, "ngx_http_auth_basic_init %d", __LINE__);
        int ret = 0;
        //domain = pk_domain_create(PK_KEY_INHERIT | PK_KEY_COPY); //TODO
        domain = pk_domain_create(0);
        assert(domain >= 0);
        int current_key = pk_domain_default_key(PK_DOMAIN_CURRENT);
        assert(current_key >= 0);
        ret = pk_domain_allow_caller2(domain, pk_current_did(), 0);
        assert(ret >= 0);
        ret = pk_domain_assign_pkey(domain, current_key, PK_KEY_COPY, 0); // required because (parts of) ngx_http_request_t lie in protected parent memory
        assert(ret >= 0);
        ret = PK_ECALL_REGISTER(ngx_http_auth_basic_handler_private, domain);
        assert(ret >= 0);
        //pk_sysfilter_domain(domain, SYS_getuid, SYSCALL_DENIED, NULL);

        //TODO register private file. but it isnt known yet. => moved to request handler

        //register localstorage filters for domain
        if(strcmp(filter, "nginx") == 0){
            sf_localstorage_filters_init(domain);
        }

        //TODO release child depending on threat model
        //ret = pk_domain_release_child(domain);
        //assert(ret >= 0);
    }
