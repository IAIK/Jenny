...
static int domain = -1;
//GEN_ALL(ngx_http_gzip_filter_deflate, static ngx_int_t, ngx_http_request_t *r, ngx_http_gzip_ctx_t *ctx)
GEN_ALL(deflate, int, z_streamp strm, int flush)
...
ngx_http_gzip_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    for ( ;; ) {
        ...
        for ( ;; ) {
            ...
            //}else{
            //    rc = ECALL(ngx_http_gzip_filter_deflate(r, ctx));
            //}
            rc = ngx_http_gzip_filter_deflate(r, ctx);
...
ngx_http_gzip_filter_deflate(ngx_http_request_t *r, ngx_http_gzip_ctx_t *ctx)
{
    ...
    //fprintf(stderr, "ngx_http_gzip_filter_deflate:%d", __LINE__);

    //rc = deflate(&ctx->zstream, ctx->flush);
    if(domain == -1){
        rc = deflate(&ctx->zstream, ctx->flush);
    }else{
        rc = ECALL(deflate(&ctx->zstream, ctx->flush));
    }
...
ngx_http_gzip_filter_init(ngx_conf_t *cf)
{
    ...
    if ((mech && strcmp(mech, "none") != 0) || (filter && strcmp(filter, "none") != 0)) {
        ...
        assert(ret >= 0);
        //ret = PK_ECALL_REGISTER(ngx_http_gzip_filter_deflate, domain);
        ret = PK_ECALL_REGISTER(deflate, domain);
        assert(ret >= 0);
        for (long sysno = 0; sysno < NUM_DOMAIN_FILTERS; sysno++) {
            //if(sysno == SYS_munmap){
            //    continue;
            //}
            int ret = pk_sysfilter_domain(domain, sysno, SYSCALL_DENIED, NULL);
            //if(errno == EINVAL){}
            assert(ret == 0);
        }
    }
