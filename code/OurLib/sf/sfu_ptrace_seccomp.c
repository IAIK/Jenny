...
static void *_tracee_wrapper(void *arg)
{
    ...
    if (_register_bpf_type(SF_PTRACE_SECCOMP) < 0) {
...
int _sf_mechanism_ptrace_seccomp(sf_tracee_function *start)
{
    ...
    return pk_sysfilter_tracer(sf_ptrace_seccomp_tracer, tracee);
}
//------------------------------------------------------------------------------

PK_API void *_ptrace_seccomp_tracee_wrapper_ctor(void *arg) {
    sf_tracee_function* start = (sf_tracee_function*)arg;
...
//------------------------------------------------------------------------------
 ...
 __attribute__((naked)) static void * _tracee_wrapper_ctor_asm(void *arg) {
    asm volatile(
      "addq $-8, %rsp\r\n"
      "call _ptrace_seccomp_tracee_wrapper_ctor\r\n"
      "mov %rax, %rsp\r\n" // restore parent's stack pointer
      "movq $0, %rax\r\n"  // Prepare return value for _sf_mechanism_ptrace_seccomp_ctor_asm
      "ret\r\n"            // ret will continue at the end of the tracer's ctor
                           // (i.e., the call site of _sf_mechanism_ptrace_seccomp_ctor_asm)
                           // and drop into main
    );
}
//------------------------------------------------------------------------------
...
int _sf_mechanism_ptrace_seccomp_ctor(void* stack) {
    ...
    pk_sysfilter_tracer(sf_ptrace_seccomp_tracer_ctor_asm, tracee);
...
#ifdef __x86_64__
 __attribute__((naked)) int _sf_mechanism_ptrace_seccomp_ctor_asm() {
    asm volatile(
        "mov %rsp, %rdi\r\n"  // save original stack pointer
        "addq $-8, %rsp\r\n"
        "call _sf_mechanism_ptrace_seccomp_ctor\r\n"
    );
}
//-------------------------------------------------------------------------------
#else
