...
void pkey_isolation_child_success(volatile uintptr_t* mem, int shared_pkey) {
  ...
  for (size_t i = 0; i < NUM_WORDS_TO_TEST/*256/sizeof(uintptr_t)*/; i++) {
...
void test_pkey_isolation() {
  ...
  assert(0 == pk_domain_allow_caller2(did, pk_current_did(), 0));
  ...
  volatile uintptr_t val2 = 0;
  for (size_t i = 0; i < NUM_WORDS_TO_TEST/*PAGESIZE/sizeof(uintptr_t)*/; i++) {
    /* We're accessing child-private memory */
    ACCESS_MUST_FAIL(val = child_stack[i]);
    val2 += val; // to prevent compiler warnings/optimizations
  }
