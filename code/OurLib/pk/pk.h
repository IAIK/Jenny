...
#define PK_DROP_CHILD                1
#define PK_SYSMODULE                 2
 ...
 *        0
 *        @c PK_DROP_CHILD
 *           Create an additional child domain which will be allowed to call the root domain
 *           at the entry point provided via *args.
 * 	      @c PK_SYSMODULE
 * 			 Activates the sysfilter kernel module for intercepting system calls.
 * @return
 *        0 or a positive child did on success, or -1 on error, and errno is set to:
 *        @c EACCES
 *            Todo: specify
 */
int pk_init(int flags, void* arg1, void* arg2);
...
void pk_name_range(void * addr, size_t len, char * name);
...
int pk_madvise(void *addr, size_t length, int advice);
int pk_madvise2(int did, void* addr, size_t len, int advice);
...
int pk_domain_register_ecall3(int did, int ecall_id, void* entry, char * name);
 ...
 */
void pk_pthread_exit(void *retval) __attribute__ ((__noreturn__));
...
int pk_set_binary_scanning(uint64_t arg1);
int pk_api_generic(uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4, uint64_t arg5, uint64_t arg6);

inline int pk_api_generic_register_private_file(int domain, char * path){
    return pk_api_generic(1, (uint64_t)domain, (uint64_t)path, 0, 0, 0);
}
...
char * pk_print_keys(int did);
void pk_my_debug_check();

extern int domain_of_main;
