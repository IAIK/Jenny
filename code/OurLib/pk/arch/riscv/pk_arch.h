...
#define FULL_ACCESS_BIT 0x4000000000000000  // 1 << 62
...
#define CSRR(csr_id) ({uint64_t csrr_ret; asm volatile ("csrr %0, %1" : "=r"(csrr_ret) : "i"(csr_id)); csrr_ret;}) // GCC statement expression
...
#define CURRENT_DID ({ assert_ifdebug(pk_trusted_tls.init); _read_pkru_reg().sw_did;})

FORCE_INLINE char * pk_sprint_reg_arch(pkru_config_t reg){
    static char buf[256];
    int len = 0;
    len += sprintf(buf + len, "raw = 0x%zx, ", PKRU_TO_INT(reg));
    len += sprintf(buf + len, "(mode=%d, full_access=%d), did = %4u, ", reg.mode, reg.full_access, reg.sw_did);
    len += sprintf(buf + len, "keys = [%4u](wd=%1u) [%4u](wd=%1u) [%4u](wd=%1u) [%4u](wd=%1u)", 
...
#define _pk_is_pkey_loaded_arch(pkey, pkru) ({ \
    bool _ret = false; \
    uint _pkey = pkey; \
    if (pkru.slot_0_mpkey == _pkey || \
        pkru.slot_1_mpkey == _pkey || \
        pkru.slot_2_mpkey == _pkey || \
        pkru.slot_3_mpkey == _pkey) { \
        _ret = true; \
    } \
    _ret; \
})
...
#define SYSCALL_REENTRY ((void *)CSRR(CSR_UEPC))
#define SYSCALL_STACK   ((uint64_t *)CSRR(CSR_USCRATCH))
...
.macro goto_exception_handler_preserve_ra id type
    // this is similar to goto_exception_handler
    // except that we do not change the ra register
    li       reg_a1_id, \id
    li       reg_a2_type, \type
    loadword t5 _pk_exception_handler
.endm
